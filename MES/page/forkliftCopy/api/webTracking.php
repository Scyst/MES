    ...
    switch ($action) {
        ...
        case 'get_history':
            ...
            $sql = "SELECT h.location_type, h.lat, h.lng, h.indoor_x, h.indoor_y, h.recorded_at, h.last_location
                    FROM dbo.FORKLIFT_HISTORY_LOGS h WITH (NOLOCK)
                    JOIN dbo.FORKLIFTS f WITH (NOLOCK) ON h.forklift_id = f.id
        ...
        case 'get_heatmap':
            ...
            $sql = "SELECT location_type, lat, lng, indoor_x, indoor_y FROM dbo.FORKLIFT_HISTORY_LOGS WITH (NOLOCK)
