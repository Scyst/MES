...
// 🔴 --- SANDBOX OVERRIDES (บังคับชี้ไปที่ตารางฝั่งทดสอบ) ---
$transferTable   = 'STOCK_TRANSFER_ORDERS_TEST';
$transTable      = 'STOCK_TRANSACTIONS_TEST';
$spUpdateOnhand  = 'sp_UpdateOnhandBalance_TEST';
$lotSerialsTable = 'LOT_SERIALS_TEST';
$scanJobsTable   = 'QR_SCAN_JOBS_TEST';

// ตาราง Master Data ใช้อ่านจากของจริง
$itemTable   = 'ITEMS';
$locTable    = 'LOCATIONS';
$usersTable  = 'USERS';
$routesTable = 'MANUFACTURING_ROUTES';
// --------------------------------------------------------
...
try {
    switch ($action) {
        ...
        case 'cancel_label':
            ...
            // 🔴 เรียกใช้ SP _TEST
            $stmt = $pdo->prepare("EXEC dbo.sp_CancelAndRollbackTag_TEST @transfer_uuid = ?, @user_id = ?");
        ...
        case 'confirm_transfer':
            ...
            $pdo->commit();
            echo json_encode(['success' => true, 'message' => 'รับของเข้าสำเร็จ! สต็อกถูกอัปเดตแล้ว (Sandbox)']);
            break;
        ...
        case 'create_batch_transfer_orders':
            ...
            $sqlReserve = "
                MERGE INTO " . $lotSerialsTable . " WITH (HOLDLOCK) AS T
        ...
        case 'get_pending_shipments':
            ...
            $totalSql = "SELECT COUNT(*) FROM " . $transTable . " t
                        JOIN " . $itemTable . " i ON t.parameter_id = i.item_id
                        ...
                        LEFT JOIN " . $usersTable . " u ON t.created_by_user_id = u.id
                        {$whereClause}";
        ...
        case 'check_lot_status':
            ...
            if (!empty($scan_id)) {
                $sql = "SELECT is_used, job_data FROM " . $scanJobsTable . " WHERE scan_id = ?";
                $stmt = $pdo->prepare($sql);
            ...
            $itemStmt = $pdo->prepare("SELECT item_id FROM " . $itemTable . " WHERE sap_no = ?");
        ...
        case 'get_locations_for_qr':
            ...
            $stmt = $pdo->query("SELECT location_id, location_name FROM " . $locTable . " WHERE is_active = 1 ORDER BY location_name");
        ...
        case 'get_next_serial':
            ...
            $sql = "
                MERGE INTO " . $lotSerialsTable . " AS T
                USING (SELECT ? AS parent_lot) AS S
                ON (T.parent_lot = S.parent_lot)
                WHEN MATCHED THEN
                    UPDATE SET 
        ...
        case 'create_scan_job':
            ...
            for ($i = 0; $i < $max_tries; $i++) {
                ...
                $checkStmt = $pdo->prepare("SELECT COUNT(*) FROM " . $scanJobsTable . " WHERE scan_id = ?");
            ...
            $sql = "INSERT INTO " . $scanJobsTable . " (scan_id, job_data, created_at, is_used) VALUES (?, ?, GETDATE(), 0)";
            $stmt = $pdo->prepare($sql);
        ...
        case 'get_scan_job_data':
            ...
            $sql = "SELECT job_data FROM " . $scanJobsTable . " WHERE scan_id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$scan_id]);
