<?php
// MES/page/productionCopy/api/inventoryManage.php
require_once __DIR__ . '/../../db.php';
require_once __DIR__ . '/../../components/init.php';

header('Content-Type: application/json; charset=utf-8');

// 🔴 --- SANDBOX OVERRIDES (บังคับชี้ไปที่ตารางและ SP ฝั่งทดสอบ) ---
$transactionsTable   = 'STOCK_TRANSACTIONS_TEST';
$onhandTable         = 'INVENTORY_ONHAND_TEST';
$spUpdateOnhand      = 'sp_UpdateOnhandBalance_TEST';
$spExecuteProduction = 'sp_ExecuteProduction_TEST';
// ---------------------------------------------------------------
    ...
    switch ($action) {
        ...
        case 'execute_receipt':
            ...
            try {
                ...
                $spStock = $pdo->prepare("EXEC dbo." . $spUpdateOnhand . " @item_id = ?, @location_id = ?, @quantity_to_change = ?");
                $spStock->execute([$item_id, $to_location_id, $quantity]);
                
                $transSql = "INSERT INTO " . $transactionsTable . " (parameter_id, quantity, transaction_type, to_location_id, created_by_user_id, notes, reference_id, transaction_timestamp) VALUES (?, ?, 'RECEIPT', ?, ?, ?, ?, ?)";
                ...
                $pdo->commit();
                writeLog($pdo, $logType, 'INVENTORY_API_TEST', $item_id, null, null, $logDetail);
         ...
         case 'get_stock_inventory_report':
            ...
            $dataSql = "
                ...
                ItemGroup AS (
                    ...
                    LEFT JOIN " . $onhandTable . " h ON fi.item_id = h.parameter_id
        ...
        case 'get_wip_onhand_report':
            ...
            $totalSql = "SELECT COUNT(*) FROM ". $onhandTable ." h JOIN ". ITEMS_TABLE ." i ON h.parameter_id = i.item_id JOIN ". LOCATIONS_TABLE ." l ON h.location_id = l.location_id {$whereClause}";
            $totalStmt = $pdo->prepare($totalSql);
            $totalStmt->execute($params);
            $total = (int)$totalStmt->fetchColumn();
        ...
        case 'execute_production':
            ...
            try {
                ...
                echo json_encode([
                    'success' => true, 
                    'message' => "บันทึกการผลิตและตัดสต็อกวัตถุดิบสำเร็จ (Sandbox Mode)"
                ]);
        ...
        case 'update_transaction':
            ...
            try {
                ...
                $stmt = $pdo->prepare("SELECT created_by_user_id FROM " . $transactionsTable . " WHERE transaction_id = ?");
                $stmt->execute([$transaction_id]);
                ...
                $stmt = $pdo->prepare("SELECT * FROM " . $transactionsTable . " WITH (UPDLOCK) WHERE transaction_id = ?");
                $stmt->execute([$transaction_id]);
                ...
                if (strpos($old_transaction['transaction_type'], 'PRODUCTION_') === 0) {
                    ...
                    $getConsumeSql = "SELECT parameter_id, quantity, from_location_id FROM " . $transactionsTable . " WHERE notes = ?";
                    ...
                    $deleteConsumeSql = "DELETE FROM " . $transactionsTable . " WHERE notes = ?";
                    ...
                    $updateSql = "UPDATE " . $transactionsTable . " 
                    ...
                    if (in_array($new_count_type, ['FG', 'NG', 'SCRAP'])) {
                        ...
                        if (!empty($components)) {
                            $consumeSql = "INSERT INTO " . $transactionsTable . " (
                    ...
                    $sql = "UPDATE " . $transactionsTable . " SET quantity=?, from_location_id=?, to_location_id=?, reference_id=?, notes=?, transaction_timestamp=? WHERE transaction_id=?";
                    $stmt = $pdo->prepare($sql);
                ...
                $pdo->commit();
                writeLog($pdo, 'UPDATE', 'INVENTORY_API_TEST', $transaction_id, null, null, "Updated transaction in Sandbox.");
        ...
        case 'delete_transaction':
            ...
            try {
                ...
                $deleteStmt = $pdo->prepare("DELETE FROM " . $transactionsTable . " WHERE transaction_id = ?");
                $deleteStmt->execute([$transaction_id]);

                $pdo->commit();
                writeLog($pdo, 'DELETE', 'INVENTORY_API_TEST', $transaction_id);
                echo json_encode(['success' => true, 'message' => 'Transaction deleted successfully from Sandbox.']);
        ...
        case 'adjust_single_stock':
            ...
            try {
                ...
                $onhandStmt = $pdo->prepare("SELECT quantity FROM " . $onhandTable . " WHERE parameter_id = ? AND location_id = ?");
                ...
                $transSql = "INSERT INTO " . $transactionsTable . " (parameter_id, quantity, transaction_type, to_location_id, created_by_user_id, notes) VALUES (?, ?, 'ADJUSTMENT', ?, ?, ?)";
                $transStmt = $pdo->prepare($transSql);
                $transStmt->execute([$item_id, $variance, $location_id, $currentUser['id'], $notes]);

                $pdo->commit();
                writeLog($pdo, 'QUICK_ADJUST', 'INVENTORY_API_TEST', $item_id, null, null, "Location: {$location_id}, New Qty: {$physical_count}, Variance: {$variance}");
                echo json_encode(['success' => true, 'message' => 'Stock adjusted successfully in Sandbox.']);
        ...
        case 'get_stock_onhand':
            ...
            $stockStmt = $pdo->prepare("SELECT quantity FROM " . $onhandTable . " WHERE parameter_id = ? AND location_id = ?");
