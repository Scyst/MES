    ...
    switch ($action) {
        ...
        case 'read_daily':
            ...
            $sql = "SELECT 
                    ...
                    FROM dbo.MANPOWER_EMPLOYEES_TEST E WITH (NOLOCK)
                    LEFT JOIN dbo.MANPOWER_SHIFTS_TEST S_Master WITH (NOLOCK) ON E.default_shift_id = S_Master.shift_id
                    OUTER APPLY (
                        SELECT TOP 1 * FROM dbo.MANPOWER_CATEGORY_MAPPING_TEST M WITH (NOLOCK)
                    ...
                    LEFT JOIN dbo.MANPOWER_SHIFTS_TEST S WITH (NOLOCK) ON L.shift_id = S.shift_id
        ...
        case 'read_summary':
            ...
            $spName = 'sp_GetManpowerDashboardData_TEST'; // เปลี่ยนเป็น _TEST
        ...
        case 'clear_day':
            ...
            if (!empty($line) && $line !== 'ALL') {
                $sql = "DELETE L FROM dbo.MANPOWER_DAILY_LOGS_TEST L
                ...
                $stmt = $pdo->prepare("DELETE FROM dbo.MANPOWER_DAILY_LOGS_TEST WHERE log_date = ?");
                $stmt->execute([$date]);
            }
        ...
        case 'update_log_status':
            ...
            // Get Date for Recalc
            $stmtDate = $pdo->prepare("SELECT log_date FROM dbo.MANPOWER_DAILY_LOGS_TEST WHERE log_id = ?");
            ...
            // Recalc (เปลี่ยนไปใช้ _TEST)
            if ($rowDate) {
                $pdo->prepare("EXEC sp_CalculateDailyCost_TEST @StartDate = ?, @EndDate = ?")->execute([$rowDate['log_date'], $rowDate['log_date']]);
            }
        ...
        case 'delete_log':
            ...
            $stmtCheck = $pdo->prepare("SELECT status, log_date FROM dbo.MANPOWER_DAILY_LOGS_TEST WHERE log_id = ?");
            ...
            $pdo->beginTransaction();
            $pdo->prepare("DELETE FROM dbo.MANPOWER_DAILY_LOGS_TEST WHERE log_id = ?")->execute([$logId]);
            ...
            // Recalc (เปลี่ยนไปใช้ _TEST)
            $pdo->prepare("EXEC sp_CalculateDailyCost_TEST @StartDate = ?, @EndDate = ?")->execute([$log['log_date'], $log['log_date']]);
        ...
        case 'read_range_report':
            ...
            $spName = 'sp_GetExecutiveRangeReport_TEST'; // เปลี่ยนไปใช้ SP _TEST
        ...
        case 'update_log':
            ...
            $stmtCheck = $pdo->prepare("SELECT log_id FROM dbo.MANPOWER_DAILY_LOGS_TEST WHERE (log_id = ? AND log_id != '0') OR (emp_id = ? AND log_date = ?)");
            ...
            if ($existing) {
                ...
                $sql = "INSERT INTO dbo.MANPOWER_DAILY_LOGS_TEST 
            ...
            // Recalc (เปลี่ยนไปใช้ SP _TEST)
            $recalcStmt = $pdo->prepare("EXEC sp_CalculateDailyCost_TEST @StartDate = ?, @EndDate = ?");
        ...
        case 'compare_cost':
            ...
            $sql = "EXEC sp_CompareLaborCost_Logic_TEST @StartDate = :start, @EndDate = :end"; // เปลี่ยนเป็น _TEST
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                ':start' => $startDate,
                ':end'   => $endDate
