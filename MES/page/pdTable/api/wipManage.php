...
try {
    switch ($action) {
        case 'log_wip_entry':
            ...
            $checkSql = "SELECT COUNT(*) FROM " . LEGACY_PARAMETER_TABLE . " WHERE part_no = ? AND model = ?";
        ...
        case 'get_wip_report':
            ...
            $dataSql = "
                WITH TotalIn AS (
                    ...
                    LEFT JOIN " . LEGACY_PARAMETER_TABLE . " p ON ISNULL(tin.line, tout.line) = p.line AND ISNULL(tin.model, tout.model) = p.model AND ISNULL(tin.part_no, tout.part_no) = p.part_no
                ), NumberedRows AS (
        ...
        case 'get_wip_report_by_lot':
            ...
            $dataSql = "
                ...
                FinalResult AS (
                    ...
                    LEFT JOIN TotalIn ti ON MasterLots.line = ti.line AND MasterLots.model = ti.model AND MasterLots.part_no = ti.part_no AND MasterLots.lot_no = ti.lot_no
                    LEFT JOIN TotalOut to_out ON MasterLots.line = to_out.line AND MasterLots.model = to_out.model AND MasterLots.part_no = to_out.part_no AND MasterLots.lot_no = to_out.base_lot_no
                    LEFT JOIN " . LEGACY_PARAMETER_TABLE . " p ON MasterLots.line = p.line AND MasterLots.model = p.model AND MasterLots.part_no = p.part_no
                    $whereClause
                ),
                NumberedRows AS (
        ...
        case 'get_stock_count':
            ...
            $countSql = "SELECT COUNT(*) FROM " . LEGACY_PARAMETER_TABLE . " p $paramWhereClause";
            $totalStmt = $pdo->prepare($countSql);
