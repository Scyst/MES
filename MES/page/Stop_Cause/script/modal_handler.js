...
// ประกาศเผื่อไว้ในกรณีที่ไฟล์อื่นไม่ได้นิยามไว้
if (typeof API_URL === 'undefined') {
    window.API_URL = 'api/stopCauseManage.php'; 
}
...
async function openEditModal(id, triggerEl) {
    ...
    try {
        ...
        if (typeof showToast === 'function') showToast('Failed to fetch details for editing.', '#dc3545');
    } finally {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addForm) {
        addForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (typeof showToast === 'function') showToast('An error occurred while adding data.', '#dc3545');
            } finally {
                if (typeof hideSpinner === 'function') hideSpinner();
            }
        });
    ...
    if (editForm) {
        editForm.addEventListener('submit', async (e) => {
            ...
            // ✅ แก้ไข Logic ให้ดึงค่าจาก payload.cause แทน cause_category
            const baseCause = payload.cause || payload.edit_cause; 
            ...
            delete payload.cause_other;
            delete payload.edit_cause; // เคลียร์ขยะออกเผื่อมีการตั้งชื่อซ้ำ
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (modalInstance) {
                        modalElement.addEventListener('hidden.bs.modal', () => {
                            if (typeof fetchStopData === 'function' && typeof currentPage !== 'undefined') {
                ...
                if (typeof showToast === 'function') showToast('An error occurred while updating data.', '#dc3545');
            } finally {
                if (typeof hideSpinner === 'function') hideSpinner();
            }
        });
