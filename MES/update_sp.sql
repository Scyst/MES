...
BEGIN
    ...
    WHERE st.transaction_type = 'PRODUCTION_FG' 
      ...
      AND (@TeamGroup = 'ALL' OR u.team_group = @TeamGroup)
    ...
    WHERE st.transaction_type = 'SCRAP' 
      ...
      AND (@TeamGroup = 'ALL' OR u.team_group = @TeamGroup);
        ...
        -- [E] Labor (Dynamic via fn_GetManpowerSummary_V2)
    SELECT 
        @Val_Labor_Base = ISNULL(SUM(CASE WHEN rate_type = 'DAILY' THEN Normal_Cost ELSE 0 END), 0),
        @Val_Labor_OT   = ISNULL(SUM(OT_Cost), 0)
    FROM dbo.fn_GetManpowerSummary_V2(@EntryDate)
    WHERE (@Section = 'ALL' OR display_section = @Section)
      AND (@TeamGroup = 'ALL' OR hc_group = @TeamGroup);
