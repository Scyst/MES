...
async function backupFtp() {
    ...
    const localBackupDir = path.join('E:', 'MES', 'Backups', `TestBackup_auth_${timestamp}`);
    ...
    try {
        ...
        console.log(`Connected. Starting backup of /auth to ${localBackupDir}`);
        await client.downloadToDir(localBackupDir, '/auth');
