...
try {
    ...
    // 1.5 Get Tasks (Bookings) for UI Mockup
    else if ($action === 'get_tasks') {
        $stmt = $pdo->query("SELECT booking_id as task_id, start_time as created_at, status, usage_details as pickup_location FROM " . FORKLIFT_BOOKINGS_TABLE . " ORDER BY booking_id DESC");
