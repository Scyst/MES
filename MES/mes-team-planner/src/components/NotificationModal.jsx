    ...
    <>
      ...
      <div className="fixed top-14 right-4 md:top-16 md:right-6 w-[calc(100%-2rem)] md:w-80 max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl z-[110] overflow-hidden animate-slide-up border border-slate-200 dark:border-slate-800 flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setIsReadAll(true)} className="text-[10px] text-indigo-600 dark:text-indigo-400 font-medium hover:underline">Mark all as read</button>
          ...
          {activities.length > 0 ? (
            <div className="divide-y divide-slate-100 dark:divide-slate-800">
              {activities.slice(0, 10).map((act, i) => (
                <div key={act.Id || i} className={`p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer ${(!isReadAll && i < 3) ? 'bg-indigo-50/30 dark:bg-indigo-500/5' : ''}`}>
                  <div className="flex gap-3">
                    ...
                    {(!isReadAll && i < 3) && <div className="w-2 h-2 rounded-full bg-rose-500 shrink-0 mt-1.5 ml-auto"></div>}
                  </div>
                </div>
              ))}
            </div>
