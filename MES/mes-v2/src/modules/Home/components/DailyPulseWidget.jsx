      ...
      <div className="grid grid-cols-3 gap-3 flex-1">
        {[1, 2, 3].map(pid => {
          ...
          return (
            ...
            >
              <div className="text-xs font-bold text-gray-500 mb-2">{info.label}</div>
              
              {isDone ? (
                <div className="flex flex-col items-center">
                  <span className="text-3xl leading-none">{emojis[log.mood]}</span>
                  <span className="text-[10px] text-green-600 font-semibold mt-2 bg-green-100 px-2 py-0.5 rounded-full">บันทึกแล้ว</span>
                </div>
              ) : (
                <div className="flex flex-col items-center text-gray-400">
                  <PlusCircle size={28} strokeWidth={1.5} className="mb-2 opacity-50" />
                  <span className="text-[10px]">กดบันทึก</span>
                </div>
              )}
            </button>
          );
