...
import { Truck, Search, Plus, Filter, AlertCircle, PackageCheck, FileText, Download } from 'lucide-react';
...
export default function ShippingLoading() {
  ...
  const fetchShipping = async () => {
    try {
      setLoading(true);
      setError(null);
      const url = new URL(`${API_BASE_URL}/shipping.php`, window.location.href);
      ...
      if (json.success) {
        setShippingRecords(json.data || []);
      } else {
        throw new Error(json.message || 'Failed to fetch shipping records');
      }
    } catch (err) {
  ...
  const filtered = shippingRecords.filter(s => 
    !searchTerm || 
    s.invoice_no?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    s.container_no?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="h-full flex flex-col space-y-4 lg:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage outbound logistics and loading reports</p>
        </div>
        ...
        <div className="flex w-full sm:w-auto items-center gap-2 sm:gap-3">
          ...
          <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-sm font-medium flex items-center gap-2 shadow-sm transition-colors">
            <Plus size={16} />
            <span className="hidden sm:inline">New Loading Report</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col">
        {loading && shippingRecords.length === 0 ? (
          ...
          <div className="flex-1 flex flex-col justify-center items-center text-slate-500 dark:text-slate-400 p-8 text-center">
            ...
            <p className="text-lg font-medium">No Shipping Records</p>
            <p className="text-sm max-w-md mt-1">There are no shipping records matching your criteria.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-700">
                <tr>
                  ...
                  <th className="px-6 py-4 font-semibold">Seal No</th>
                  <th className="px-6 py-4 font-semibold">PO / Booking No</th>
                ...
                {filtered.map(s => (
                  <tr key={s.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-700/20 transition-colors">
                    <td className="px-6 py-4 text-slate-900 dark:text-white font-medium">
                      {s.loading_date ? new Date(s.loading_date).toLocaleDateString('en-GB') : '-'}
                    </td>
                    ...
                    <td className="px-6 py-4 text-slate-600 dark:text-slate-400">
                      <div>{s.po_number || '-'}</div>
                      <div className="text-xs text-slate-500">{s.booking_no || ''}</div>
                    </td>
